...
<body>
    ...
    <div class="container__container_lista">
        <div class="text-center header__table"> <h1> DENUNCIAS ABAIXO </h1></div>
        <div class="container">
        <table class="table table-striped">
            <thead>
              <tr>
                <th scope="col" class="tittle__table">Mensagem</th>
                <th scope="col" class="tittle__table">RA</th>
              </tr>
            </thead>
            <tbody>
    <?php
...
$read = $conexao->query("SELECT mensagem, RA, id, email from denuncia");
while ($row = $read->fetch(PDO::FETCH_OBJ)) {
  echo "<tr class='box__container'>";
  echo " <td> <a href='email.php?RA=$row->RA&email=$row->email'>$row->mensagem</td>";
  echo " <td>$row->RA</td>";
          echo " </tr>";
}
